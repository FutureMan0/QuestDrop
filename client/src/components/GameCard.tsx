...
const GameCard = ({
  ...
  useEffect(() => {
    if (!isTouchDevice || !mobileQuickActionsOpen) return;
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(autoFadeTimer);
  }, [isTouchDevice, mobileQuickActionsOpen]);
  ...
  const handleDetailsClick = ({ forceNavigate = false }: { forceNavigate?: boolean } = {}) => {
    if (isTouchDevice && !forceNavigate && !mobileQuickActionsOpen) {
    ...
    >
      <div className="relative">
        ...
        <div
          ...
          className={`pointer-events-none absolute inset-0 flex items-center justify-center gap-2 bg-black/60 opacity-0 transition-opacity duration-200 group-hover:pointer-events-auto group-hover:opacity-100 focus-within:pointer-events-auto focus-within:opacity-100 ${isTouchDevice && mobileQuickActionsOpen ? "pointer-events-auto opacity-100" : ""} ${isCarouselLayout ? "" : "rounded-t-md"}`}
        >
          <Tooltip>
          ...
          {isTouchDevice && mobileQuickActionsOpen && (
            <div className="absolute inset-x-3 bottom-3 rounded-md border border-border/60 bg-background/75 p-2 text-xs text-foreground shadow-sm backdrop-blur-sm">
              ...
              <Button
                ...
                disabled={addGameMutation.isPending}
              >
                {addGameMutation.isPending ? "Requesting..." : "Request"}
              </Button>
            </div>
          )}
